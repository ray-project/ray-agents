...
Waits up to five minutes by default (`timeoutMs` / `timeout`); a fast pause still returns as soon as it lands. If the host has not finished by the deadline the call throws a timeout error, but the pause itself carries on — `getInfo()` / `get_info()` reports `paused` once it lands.
