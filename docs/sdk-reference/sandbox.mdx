...
Waits up to 30 seconds by default (`timeoutMs` / `timeout`). If the host has not finished by then the call throws a timeout error, but the pause itself carries on — `getInfo()` / `get_info()` reports `paused` once it lands.

<CodeGroup>
```typescript TypeScript
await sandbox.pause()
await sandbox.pause({ timeoutMs: 120_000 })
```

```python Python
sandbox.pause()
sandbox.pause(timeout=120)
```
