...
description: See every HTTP and HTTPS connection a sandbox made on ports 80 and 443 — allowed, blocked, or failed — and every secret-bearing request, in one timeline.
---

The network log answers a question you can't answer from inside an untrusted sandbox: **what did it actually reach on the network?** Every outbound HTTP and HTTPS connection on ports 80 and 443 passes through Superserve, so the log is recorded by the platform — code inside the sandbox can't see it or tamper with it.
